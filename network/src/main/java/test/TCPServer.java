...
public class TCPServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 1-1. TIME_WAIT 상태에서도 소켓 포트 번호를 할당 가능하겠금(기본설정)
			serverSocket.setReuseAddress(true);
			
			
			// 2. 바인딩(binding)
			// Socket에 InetSocketAddress(IP Address + Port)를 바인딩한다.
			// IPAddress(0.0.0.0) " : 특정 호스트 1111ip를 바인딩 하지 않는다
			// backlog : 요청queue
			...
			System.out.println("[server] connected by client[" + remoteAddress + "]:[" + remotePort + "]");
			...
			try {
				...
				while(true) {
					...
					if(readByteCount == -1) {
						//클라이언트가 정상적으로 종료, close() 명시적으로 호출
